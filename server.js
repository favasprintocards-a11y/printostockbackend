...
app.post('/api/transactions', async (req, res) => {
    const { productId, type, quantity, party, notes, chipLayout, qtyOfSheet, keyEncoding, designParty, store, date } = req.body;
    try {
...
app.get('/api/products/:id/history', async (req, res) => {
    try {
        const history = await Transaction.find({ product: req.params.id })
            .sort({ date: -1 }); // Get full history for accurate running balance calculation
        res.json(history);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
